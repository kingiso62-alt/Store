...
export async function POST(req:Request){
 const rl=rateLimit(clientKey(req,'payment'),10,60_000);if(!rl.ok)return NextResponse.json({error:'Too many requests'},{status:429,headers:{'Retry-After':String(Math.ceil((rl.resetAt-Date.now())/1000))}});
 try{
